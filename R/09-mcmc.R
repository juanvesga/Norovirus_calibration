...
if (use_last){
  ...
  if(start_mle){
    ...
    covs<- abs(diag(log(p_mle)))*0.00000001#cov(chains_odin)
    colnames(covs)<-paste(names(p_mle))
    rownames(covs)<-paste(names(p_mle))
    ...
    # covmatrix<-list(
    #   cov(longchain[,1:(ncol(chains[[1]])-1)]),
    #   cov(longchain[,1:(ncol(chains[[1]])-1)]),
    #   cov(longchain[,1:(ncol(chains[[1]])-1)]),
    #   cov(longchain[,1:(ncol(chains[[1]])-1)]))
  ...
  covmatrix<- list(
    ...
    covmatrix<-list(
      cov(chains[[1]][,1:(ncol(chains[[1]])-1)]),
      cov(chains[[2]][,1:(ncol(chains[[2]])-1)]),
      cov(chains[[3]][,1:(ncol(chains[[3]])-1)]),
      cov(chains[[4]][,1:(ncol(chains[[4]])-1)]))
  ...
  # for (k in 1:nchains){
  # 
  #   covs<- abs(diag(log(params[k,])))*0.000001#cov(chains_odin)
  #   colnames(covs)<-paste(colnames(params))
  #   rownames(covs)<-paste(colnames(params))
  # 
  #   covmatrix[[k]]<-covs  
  #   
  # }
